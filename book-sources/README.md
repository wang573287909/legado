...
- 已自动验证：搜索与榜单默认安全节点 HTTPS 直连、独立 JavaScript VM 作用域解析、详情、目录、四种正文夹具、节点故障切换、状态 URL、评论读取映射和确定性构建。
...
搜索和发现书单默认返回当前活动安全节点的真实 HTTPS URL，由 Legado 直接取得 JSON，以避免跨 Rhino 作用域丢失大型列表响应。只有在登录面板显式启用明文末级节点且该节点成为活动节点时，才会返回 HTTP URL。这两个列表请求不在同一次请求内自动轮换节点；连接失败后可在登录面板清除或切换节点，再刷新搜索或榜单。
