...
Legado 只有在列表 HTTP 请求成功并取得响应体后才执行书籍列表规则，因此 `parseRead()` 的输入契约只有响应正文，没有 HTTP 状态码；连接失败及非成功 HTTP 状态仍由宿主网络层显示。
...
- 服务器响应不是 JSON、缺少数值 `code`、`code != 0` 或书籍字段缺失时，`parseRead()` 抛出明确错误，不把损坏响应解析成空列表；连接错误和 HTTP 错误由 Legado 在进入列表规则前报告。
